...
def model(index, N_pool):
    ...
    g_diff = np.random.normal(0, 0.03, size = 2*N_pool)
    ...
    while(survivors_0 != survivors):
        ...
        for i in range(N):
            _, _, _, K_profile, x_profile = p_profile
            if(x[0, i] not in x_profile[0] or K[i] not in K_profile):
        ...
        survivors = [list(K[:, 0]).index(i[0]) for i in K_temp]

        all_survivors=survivors
        all_params=[xAnew, xBnew], gmax_temp, K_temp
        outputFileName = 'data/2_res_new_results_gK_separated_'+str(index)+'.pkl'
